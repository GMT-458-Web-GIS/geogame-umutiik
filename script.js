// Ülkelerin verileri (isim, bayrak, koordinatlar)
const countries = [
...
// Hint butonu için buffer ekleme
...
hintButton.addEventListener("click", () => {
    ...
    if (hintButton.disabled) {
        return; // Eğer buton devre dışıysa işlem yapılmaz
    }

    const currentCountry = countries[currentCountryIndex];
    currentBuffer = L.circle(currentCountry.coordinates, {
        color: 'red',
        fillColor: '#f03',
        fillOpacity: 0.5,
        radius: 4000000 // Buffer yarıçapı (metre cinsinden)
    }).addTo(map);

    // Puan eksiltme
    score = Math.max(0, score - 5); // Puan sıfırın altına inmez
    document.getElementById('score').textContent = `Score: ${score}`;

    // Hint butonunu devre dışı bırak
    hintButton.disabled = true;
});

// Yeni tura geçtiğinde hint butonunu etkinleştir
function nextRound() {
    if (currentBuffer) {
        currentBuffer.remove();
    }

    hintButton.disabled = false; // Hint butonunu yeniden etkinleştir
